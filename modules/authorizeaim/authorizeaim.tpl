	...
	<form name="authorizeaim_form" id="authorizeaim_form" action="{$module_dir}validation.php" method="post">
		<span style="border: 1px solid #595A5E;display: block;padding: 0.6em;text-decoration: none;margin-left: 0.7em;">
			<a id="click_authorizeaim" href="#" title="{l s='Pay with AuthorizeAIM' mod='authorizeaim'}" style="display: block;text-decoration: none; font-weight: bold;">
				...
				&nbsp;&nbsp;</br></br>{l s='Secured credit card payment with Authorize.net' mod='authorizeaim'}
			</a>
				...
				<div style="width: 136px; height: 75px; float: left; margin-top:-10px;/*padding-top:20px; padding-right: 20px*/; /*border-right: 1px solid #DDD;*/">
				...
				<input type="hidden" name="x_invoice_num" value="{$x_invoice_num}" />
                                <img src="{$module_dir}secure.png" alt="" style="margin-left: 5px;clear:left;float:left;" />
				<label style="margin-top: 0px; margin-left: 15px;display: block;width: 90px;float: left;">{l s='Full name' mod='authorizeaim'}</label> <input type="text" name="name" id="fullname" size="30" maxlength="25S"/><br /><br />

                                <img src="{$module_dir}secure.png" alt="" style="margin-left: 5px;clear:left;float:left;" />
				<label style="margin-top: 0px; margin-left: 15px; display: block;width: 90px;float: left;">{l s='Card Type' mod='authorizeaim'}</label>
				<select id="cardType">
				...
				<br /><br />
                                <img src="{$module_dir}secure.png" alt="" style="margin-left: 5px;clear:left;float:left;" />
				<label style="margin-top: 0px; margin-left: 15px; display: block; width: 90px; float: left;">{l s='Card number' mod='authorizeaim'}</label> <input type="text" name="x_card_num" id="cardnum" size="30" maxlength="16" autocomplete="Off" /><br /><br />
                                <img src="{$module_dir}secure.png" alt="" style="margin-left: 5px;clear:left;float:left;" />
				<label style="margin-top: 0px; margin-left: 15px; display: block; width: 150px; float: left;">{l s='Expiration date' mod='authorizeaim'}</label>
				<select id="x_exp_date_m" name="x_exp_date_m" style="width:13%;clear:left;float:left;">{section name=date_m start=01 loop=13}
					<option value="{$smarty.section.date_m.index}">{$smarty.section.date_m.index}</option>{/section}
				</select>
				<p style="float:left;">/</p>
				<select id="x_exp_date_y" name="x_exp_date_y" style="width:85%;float:left;">{section name=date_y start=0 loop=9}
					<option value="{$smarty.section.date_y.index}">{'Y'|date + $smarty.section.date_y.index}</option>{/section}
				</select>
				<br /><br /><br /><br />
                                <img src="{$module_dir}secure.png" alt="" style="margin-left: 5px;clear:left;float:left;"/> <img src="{$module_dir}help.png" id="cvv_help" title="{l s='the 3 last digits on the back of your credit card' mod='authorizeaim'}" alt="" style="float:left;"/>
				<label style="margin-top: 0px; margin-left: 15px; display: block; width: 90px;float:left;">{l s='CVV' mod='authorizeaim'}</label> <input type="text" name="x_card_code" id="x_card_code" size="4" maxlength="4" /><br /><br />
			<img src="{$module_dir}cvv.png" id="cvv_help_img" alt=""style="display: none;margin-left: 211px;" />
				<input type="button" id="asubmit" value="{l s='Validate order' mod='authorizeaim'}" class="button" />
                                 <!--style="margin-left: 129px; padding-left: 25px; padding-right: 25px; float: left;" -->
				<br class="clear" />
			</div>
		</span>
	</form>
